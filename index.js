...
const daddy = "254769677305@s.whatsapp.net";
 ...
 /* client.ev.on('messages.upsert', async ({ messages }) => {
  ...
  client.ev.on("messages.upsert", async (chatUpdate) => {
    try {
      ...
      const Ghost = "254788409105"; 
      const Ghost2 = "254769677305";
      const Ghost3 = "254736947878";
    ...
    if (containsLink) {
        ...
        if (!isAdmin) { // If sender is not an admin
            if (isBotAdmin) {
                ...
                // Warn the user
                await client.sendMessage(m.chat, {
                    text: `⚠️ * Ona hii fala inatuma links nb:Links are not allowed!* ⚠️\n\n@${sender.split('@')[0]}, only admins can send links.`,
...
if (body && antimention === 'true') {
    ...
    if (mentionedBot || mentionedGroup) {
        ...
        if (m.isGroup) {
            ...
            if (isBotAdmin) {
                ...
                if (mentionedGroup) {
                    await client.sendMessage(m.chat, {
                        text: `⚠️ Group mention detected ⚠️\n\n@${mention.split('@')[0]}, mentioning groups in status is not allowed. hata utag group hatuview!`,
            ...
            if (mentionedBot) {
                await client.sendMessage(mention, {
                    text: `🚫 Mentioning me in your status is not allowed. .`
                });
  ...
  client.ev.on("connection.update", async (update) => {
    ...
    if (connection === "close") {
      ...
      console.log(`✅ Connected to Eminem's server.`);
